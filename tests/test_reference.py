...
def test_external_output_env_maps_new_bank_file(tmp_path: Path, monkeypatch) -> None:
    mounted_bank = tmp_path / "new-course.bank.json"
    monkeypatch.setenv("QUIZBANK_EXTERNAL_OUTPUT", str(mounted_bank))

    result = main(
        [
            "new",
            "/host/course/new-course.bank.json",
            "--id",
            "new-course",
            "--title",
            "New Course",
    ...
    assert result == 0
    assert mounted_bank.is_file()
    assert '"id": "new-course"' in mounted_bank.read_text(encoding="utf-8")
