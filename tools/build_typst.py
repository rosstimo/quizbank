#!/usr/bin/env python3
"""
Build a Typst source (.typ) from a quiz assembly YAML and your YAML item bank.

This keeps it simple on purpose:
- Supports mcq_one, mcq_multi, true_false, numeric, short_answer
- Does NOT convert Markdown to Typst; it prints your text as-is
  (good enough for most stems; we can get fancy later)

Usage:
  python tools/build_typst.py quizzes/quiz-example.yaml --out build/typst/quiz-example.typ

Then compile:
  typst compile build/typst/quiz-example.typ
"""
...
def t_escape(s: str) -> str:
    # Light escaping so Typst doesn't choke on braces
...
def render_item_typst(n: int, it: Dict[str, Any]) -> str:
    t = it.get("type")
    stem = t_escape(it.get("stem", "").rstrip())
    pts = it.get("points", 0)
    out = []
...
def build_typst(quiz: Dict[str, Any], items: List[Dict[str, Any]]) -> str:
    title = t_escape(quiz.get("title") or quiz.get("id") or "Quiz")
    instr = t_escape((quiz.get("instructions") or "").rstrip())
    ...
    lines.append("")
    for n, it in enumerate(items, start=1):
        ans = t_escape(answer_for(it))
        sol = t_escape((it.get("solution") or "").strip())
